...
static void parse_device(gchar *device, InstallerWindow *self) {
    g_autoptr(GError) err = NULL;

    g_autoptr(InstallerDrive) disk = NULL;
    GList *mount_points = NULL;

    g_autoptr(GVolumeMonitor) volume_monitor = g_volume_monitor_get();
    mount_points = g_volume_monitor_get_mounts(volume_monitor);

    disk = disk_manager_parse_system_disk(self->disk_manager, device, device,
                                          mount_points, &err);

    g_list_free_full(mount_points, g_object_unref);

    if (!disk) {
        if (err) {
            g_critical("Error parsing system disk: %s", err->message);
        } else {
            g_critical("Unknown problem parsing system disk");
        }

        return;
    }

    g_message("Device: %s | Model: %s | Vendor: %s | Path: %s", disk->device,
              disk->model, disk->vendor, disk->disk->path);
}
...
static void installer_window_init(InstallerWindow *self) {
    ...
    self->image_step = gtk_image_new_from_icon_name("system-software-install",
    ...
    GtkStyleContext *header_style =
        gtk_widget_get_style_context(self->header_box);
    gtk_style_context_add_class(header_style, "header-box");
    gtk_box_pack_start(GTK_BOX(self->header_box), self->image_step, FALSE, FALSE,
                       0);
    // TODO: In the python version, the line adding the step label is commented
    // out
    ...
    gtk_box_pack_start(GTK_BOX(self->header_box), self->box_labels, TRUE, TRUE,
                       0);

    installer_window_set_vanity(self);

    GtkWidget *vanity_image = gtk_image_new_from_icon_name(
        self->vanity_image, GTK_ICON_SIZE_LARGE_TOOLBAR);
    ...
    gtk_box_pack_start(GTK_BOX(self->installer_wrap), self->header_box, FALSE,
                       FALSE, 0);
    GtkWidget *separator = gtk_separator_new(GTK_ORIENTATION_VERTICAL);
    gtk_box_pack_start(GTK_BOX(self->installer_wrap), separator, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(self->installer_wrap), self->installer_page, TRUE,
                       TRUE, 0);
    ...
    gtk_box_pack_end(GTK_BOX(self->installer_page), separator_nav, FALSE, FALSE,
                     0);
    ...
    GSList *devices = disk_manager_get_devices(self->disk_manager);
    g_slist_foreach(devices, (GFunc) parse_device, self);
}
...
void installer_window_setup_style(InstallerWindow *self) {
    ...
    if (g_str_has_prefix(theme_lower, "arc")) {
...
void installer_window_page_next(InstallerWindow *self) {
    ...
    if (self->final_step) {
        const gchar *message =
            "Installation will make changes to your disks, and could "
            "result in data loss.\nDo you wish to install?";
        GtkWidget *dialog = gtk_message_dialog_new(
            GTK_WINDOW(self), GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING,
