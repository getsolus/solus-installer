...
static void installer_window_init(InstallerWindow *self) {
  ...
  g_autoptr(InstallerPartition) part =
      installer_partition_new("/dev/sdc", "sdc3", "/", &err);
  if (!part) {
    g_critical("Error creating partition wrapper: %s", err->message);
    return;
  }

  g_autoptr(InstallerPartition) windows_part = installer_partition_new(
      "/dev/sda", "sda3", "/run/media/emaddock/7004BB4604BB0E5A", &err);
  if (!windows_part) {
    g_critical("Error creating partition wrapper: %s", err->message);
  }
}
