...
static void installer_info_get_property(GObject *obj, guint prop_id,
                                        GValue *val, GParamSpec *spec);
static void installer_info_set_property(GObject *obj, guint prop_id,
...
static void installer_info_class_init(InstallerInfoClass *klass) {
  ...
  props[PROP_OWNER] = g_param_spec_gtype(
      "owner", "Owner", "The installer window that this info belongs to",
      G_TYPE_NONE,
      G_PARAM_CONSTRUCT_ONLY | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_READWRITE);

  props[PROP_ENABLE_GEOIP] = g_param_spec_boolean(
      "enable-geoip", "Enable GeoIP",
      "Whether or not GeoIP should be used for locale", FALSE,
      G_PARAM_CONSTRUCT_ONLY | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_READWRITE);

  props[PROP_HOSTNAME] = g_param_spec_string(
      "hostname", "Hostname", "The hostname to set for the installation", "",
      G_PARAM_CONSTRUCT_ONLY | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_READWRITE);

  props[PROP_WINDOWS_PRESENT] = g_param_spec_boolean(
      "windows-present", "Windows present",
      "Whether or not Windows is present on this system", FALSE,
      G_PARAM_CONSTRUCT_ONLY | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_READWRITE);

  props[PROP_INSTALL_BOOTLOADER] = g_param_spec_boolean(
      "install-bootloader", "Install bootloader",
      "Whether or not we should install a bootloader", FALSE,
      G_PARAM_CONSTRUCT_ONLY | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_READWRITE);

  props[PROP_INVALIDATED] = g_param_spec_boolean(
      "invalidated", "Invalidated",
      "Whether or not the installation info is invalid", FALSE,
      G_PARAM_CONSTRUCT_ONLY | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_READWRITE);

  g_object_class_install_properties(class, N_EXP_PROPERTIES, props);
}

static void installer_info_init(InstallerInfo *self) { (void)self; }
...
void installer_info_set_install_bootloader(InstallerInfo *self,
                                           gboolean value) {
  g_return_if_fail(INSTALLER_IS_INFO(self));
