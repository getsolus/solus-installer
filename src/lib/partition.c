...
enum { PROP_EXP_0, PROP_DISK, PROP_PARTITION, N_EXP_PROPERTIES };
...
static void installer_partition_get_property(GObject *obj, guint prop_id,
                                             GValue *val, GParamSpec *spec);
static void installer_partition_set_property(GObject *obj, guint prop_id,
                                             const GValue *val,
                                             GParamSpec *spec);
...
static void installer_partition_class_init(InstallerPartitionClass *klass) {
  ...
  props[PROP_DISK] = g_param_spec_string(
      "disk", "Disk", "Path of the disk the partition is on", "",
      G_PARAM_CONSTRUCT_ONLY | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_READWRITE);

  props[PROP_PARTITION] = g_param_spec_string(
      "partition", "Partition", "Name of the partition", "",
      G_PARAM_CONSTRUCT_ONLY | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_READWRITE);

  g_object_class_install_properties(class, N_EXP_PROPERTIES, props);
}

static void installer_partition_init(InstallerPartition *self) { (void)self; }
...
InstallerPartition *installer_partition_new(const gchar *disk,
  ...
  GObject *obj = g_object_new(INSTALLER_TYPE_PARTITION, "disk", disk,
                              "partition", part, NULL);
  ...
  BDPartSpec *part_spec =
      bd_part_get_part_spec(self->disk, self->partition, err);
  if (!part_spec) {
    g_object_unref(self);
    return NULL;
  }
...
void installer_partition_set_disk(InstallerPartition *self,
                                  const gchar *value) {
