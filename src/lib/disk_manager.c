...
gboolean disk_manager_mount_device(gchar *device, const gchar *mpoint,
                                   gchar *fsystem, gchar *options,
                                   GError **err) {
  ...
  if (options) {
    proc = g_subprocess_new(G_SUBPROCESS_FLAGS_NONE, err, "mount", "-t",
                            g_strdup(fsystem), g_strdup(device),
                            g_strdup(mpoint), "-o", g_strdup(options), NULL);
  } else {
    proc = g_subprocess_new(G_SUBPROCESS_FLAGS_NONE, err, "mount", "-t",
                            g_strdup(fsystem), g_strdup(device),
                            g_strdup(mpoint), NULL);
  }
...
gboolean disk_manager_umount_device(const gchar *mpoint, GError **err) {
  ...
  g_autoptr(GSubprocess) proc = g_subprocess_new(
      G_SUBPROCESS_FLAGS_NONE, err, "umount", g_strdup(mpoint), NULL);
  ...
  proc = g_subprocess_new(G_SUBPROCESS_FLAGS_NONE, err, "umount", "-l",
                          g_strdup(mpoint), NULL);
...
gchar *disk_manager_get_windows_version(DiskManager *self, const gchar *path,
                                        GError **err) {
...
gchar *disk_manager_get_windows_bootloader(DiskManager *self,
                                           const gchar *path) {
  if (!DISK_IS_MANAGER(self)) {
    return NULL;
  }
...
gchar *disk_manager_get_os_release_val(const gchar *path, const gchar *find_key,
                                       GError **err) {
  g_return_val_if_fail(path != NULL, NULL);
...
gchar *disk_manager_match_os_release_line(const gchar *line,
                                          const gchar *find_key) {
...
gchar *disk_manager_get_linux_version(const gchar *path) {
  ...
  gchar *name = disk_manager_search_for_key(
      path, os_release_paths, OS_RELEASE_PATHS_LENGTH, "PRETTY_NAME", "NAME");

  // Check that we have a name. If we don't, start looking at the
  // lsb_release files.
  if (!name) {
    name = disk_manager_search_for_key(path, lsb_release_paths,
                                       LSB_RELEASE_PATHS_LENGTH,
...
gchar *disk_manager_search_for_key(const gchar *root, const gchar **paths,
